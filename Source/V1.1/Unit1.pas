...
type
  TForm1 = class(TForm)
    ...
    FDTableEXTRUDERCONFIG_FILE: TStringField;
...
begin
  ...
  if not FileExists(file_path) then begin
    ...
    canProcess:= false;
    Form2.ShowModal;
    if FileExists(appPath+'\config.json') then canProcess:= true else showmessage('Config.json File not found !');
  end;
  if not canProcess then Exit;
...
begin
  ...
  if (filTypeFound and extruderFound and filNameFound) then begin                                                                                 /////////////////////////////////////////////////
    calculateAverages;
    generateTempOutput;
  end else showmessage('Missing informations in the G-Code file, choose your settings and click on the Refresh and generate button');
end;
...
begin
  ...
  if FileExists(appPath+'\Config\Config.sdb') then Form1.FDConnection1.Connected:=true else begin
